...
\usepackage{movie15}
\hypersetup{
  pdftitle={LONI \& LSU HPC User Environment},
...
\title{LONI \& LSU HPC User Environment}
...
\institute[2011 DFT Workshop] {\inst{}\footnotesize{User Services Consultant\\LSU HPC \& LONI\\sys-help@loni.org}}

\date[\hfill{July 23, 2011\hspace{2cm}\insertframenumber/\inserttotalframenumber}]{\scriptsize{DFT Workshop\\Louisiana State University\\Baton Rouge\\July 23, 2011}}
     
\subject{Talks}
\keywords{LONI \& LSU HPC: Computing Resources, User Environment, Job Management}
...
\begin{enumerate}
{\scriptsize
\item[$\vardiamond$] Gaussian 09
\item[$\vardiamond$] LMTO
\item[$\vardiamond$] NCIplot
\item[$\vardiamond$] Quantum Espresso
\item[$\vardiamond$] MPI implementation (Quantum Espresso is compiled with mvapich 1.1 and intel 11.1 compiler)
\item[$\vardiamond$] Visualization Packages: GaussView, Jmol, VMD, XCrysden
\item[$\vardiamond$] Other packages of interest: CPMD, GAMESS, NWCHEM
}
\end{enumerate}
...
\cline{1-1}\cline{3-6}
priority & & 64  & \multicolumn{2}{c|}{NA} & Requires permission \\\hline 
\end{tabular}
\end{center}
}
\end{colorblock}
